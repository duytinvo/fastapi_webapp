...
from core.constant import DOWNLOAD
from core.constant import DOWNLOAD_FOLDER
from db.models.users import User
from db.repository.texts import create_new_doc
from db.repository.texts import list_docs
from db.repository.texts import retreive_doc
...
from func.docs import WCgenerator
from schemas.texts import DocCreate
from sqlalchemy.orm import Session
from webapps.texts.forms import DocCreateForm

templates = Jinja2Templates(directory="templates")
router = APIRouter(include_in_schema=False)

@router.get("/wordcloudboard/")
async def wordcloudboard(request: Request, db: Session = Depends(get_db), msg: str = None):
...
def doc_detail(id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "texts/detail.html", {"request": request, "doc": doc, "img_path": os.path.join(DOWNLOAD, filename)}
    )

@router.get("/post-a-doc/")
def create_doc(request: Request, db: Session = Depends(get_db)):
...
async def create_doc(request: Request, db: Session = Depends(get_db)):
    ...
    if form.is_valid():
        try:
            ...
            return responses.RedirectResponse(f"/texts-webapp/details/{doc.id}", status_code=status.HTTP_302_FOUND)
...
def show_jobs_to_delete(request: Request, db: Session = Depends(get_db)):
    docs = list_docs(db=db)
    return templates.TemplateResponse("texts/show_texts_to_delete.html", {"request": request, "docs": docs})
